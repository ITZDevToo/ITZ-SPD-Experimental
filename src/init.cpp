...
std::string HelpMessage()
{
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 55675)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 55680)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Interzone is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Interzone is probably already running."), strDataDir));
    ...
    nAnonymizeInterzoneAmount = GetArg("-anonymizeinterzoneamount", 0);
    if(nAnonymizeInterzoneAmount > 999999) nAnonymizeInterzoneAmount = 999999;
    if(nAnonymizeInterzoneAmount < 2) nAnonymizeInterzoneAmount = 2;
