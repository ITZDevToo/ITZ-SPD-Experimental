...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeInterzoneAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeInterzoneAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeInterzoneAmount = strAnonymizeInterzoneAmount.remove(strAnonymizeInterzoneAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeInterzoneAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeInterzoneAmount*COIN) nMaxToAnonymize = nAnonymizeInterzoneAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeInterzoneAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeInterzoneAmount));
        strAnonymizeInterzoneAmount = strAnonymizeInterzoneAmount.remove(strAnonymizeInterzoneAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeInterzoneAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeInterzoneAmount == 0){
