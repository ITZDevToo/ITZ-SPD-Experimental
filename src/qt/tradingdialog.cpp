    ...
    model(0)
{
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "ITZ(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "ITZ(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    /*OrderBook Table Init*/

    /*Market History Table Init*/
    ui->MarketHistoryTable->setColumnCount(5);
    ui->MarketHistoryTable->verticalHeader()->setVisible(false);
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(ITZ)"<<"TOTAL COST(BTC");
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://future-exchange.com/api/v1.1/public/GetMarketSummary?market=btc-ITZ");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://future-exchange.com/api/v1.1/public/getorderbook?market=BTC-ITZ&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://future-exchange.com/api/v1.1/public/getmarkethistory?market=BTC-ITZ&count=100");
      return Response;
}
...
QString tradingDialog::BuyTX(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://future-exchange.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-ITZ&quantity=";
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://future-exchange.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-ITZ";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://future-exchange.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=ITZ";
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://future-exchange.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-ITZ&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>ITZ Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> ITZ");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    ui->TXSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(TXSupply,'i',8) + "</span><b> ITZ</b>");
    ...
    ui->TXDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(TXDemand,'i',8) + "</span><b> ITZ</b>");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
                ...
                case 5://show balance tab
                    ...
                    Response = GetBalance("ITZ");
                    if(Response.size() > 0 && Response != "Error"){
                        DisplayBalance(*ui->TXBalanceLabel,*ui->TXAvailableLabel_2,*ui->TXPendingLabel, QString::fromUtf8("ITZ"),Response);
                    }
                break;
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("ITZ");
...
{
    //calculate amount of BTC that can be gained from sellingITZavailable balance
    QString responseA = GetBalance("ITZ");
    QString str;
                ...
                if (SellResponseObject["success"].toBool() == false){
                    ...
                    if (ResponseObject["success"].toBool() == false){
                        ...
                        if (ResponseObject["success"].toBool() == false){
                            ...
                            QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+"ITZfor "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                        }
                    } else if (ResponseObject["success"].toBool() == true){
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+"ITZfor "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
